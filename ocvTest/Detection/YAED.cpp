...
CYAED::CYAED(void) : _times(6, 0.0), _timesHelper(6, 0.0)
{
...
void CYAED::SetParameters(Size	szPreProcessingGaussKernelSize,
...
uint inline CYAED::GenerateKey(uchar pair, ushort u, ushort v)
{
...
int CYAED::FindMaxK(const int* v) const
{
	int max_val = 0;
	int max_idx = 0;
...
int CYAED::FindMaxN(const int* v) const
{
	int max_val = 0;
	int max_idx = 0;
...
int CYAED::FindMaxA(const int* v) const
{
	int max_val = 0;
	int max_idx = 0;
...
float CYAED::GetMedianSlope(std::vector<Point2f>& med, Point2f& M, std::vector<float>& slopes)
{
	// med		: std::vector of points
	// M		: centroid of the points in med
	// slopes	: std::vector of the slopes
...
void CYAED::GetFastCenter(std::vector<Point>& e1, std::vector<Point>& e2, EllipseData& data)
{
	data.isValid = true;
...
void CYAED::DetectEdges13(Mat1b& DP, VVP& points_1, VVP& points_3)
{
...
void CYAED::DetectEdges24(Mat1b& DN, VVP& points_2, VVP& points_4)
{
	// Vector of connected edge points
	VVP contours;
...
void CYAED::FindEllipses(Point2f& center,
	VP& edge_i,
	VP& edge_j,
	VP& edge_k,
	EllipseData& data_ij,
...
Point2f CYAED::GetCenterCoordinates(EllipseData& data_ij, EllipseData& data_ik)
{
	float xx[7];
	float yy[7];
	...
	for (ushort i = 0; i < sz_i; ++i)
	{
		...
		for (ushort j = 0; j < sz_j; ++j)
		{
			...
			for (ushort k = 0; k < sz_k; ++k)
			{
				...
				if (PointDistanceSqr(data_ij.Cab, data_ik.Cab) > _fMaxCenterDistance2)
				{
					//discard
					continue;
				}
...
void CYAED::RemoveShortEdges(Mat1b& edges, Mat1b& clean)
{
	VVP contours;
	...
	size_t iContoursSize = contours.size();
	for (size_t i = 0; i < iContoursSize; ++i)
	{
		VP& edge = contours[i];
		size_t szEdge = edge.size();
...
void CYAED::PrePeocessing(Mat1b& I,
	Mat1b& DP,
	Mat1b& DN
)
{
...
void CYAED::DetectAfterPreProcessing(std::vector<Ellipse>& ellipses, Mat1b& E, Mat1f& PHI)
{
	// Set the image size
	_szImg = E.size();
	...
	// Other temporary 
	VVP points_1, points_2, points_3, points_4;		//std::vector of points, one for each convexity class
...
void CYAED::Detect(Mat1b& I, std::vector<Ellipse>& ellipses)
{
...
void CYAED::ClusterEllipses(std::vector<Ellipse>& ellipses)
{
	float th_Da = 0.1f;
	float th_Db = 0.1f;
	float th_Dr = 0.1f;
...
void CYAED::DrawDetectedEllipses(Mat3b& output, std::vector<Ellipse>& ellipses, int iTopN, int thickness)
{
